...
procedure RegistraExclusao(Tabela: String; ClausuraWhere: String);
function SetNumAditivo(IdServidor, IdLanctoCtroOrig: String): String;
function ProxNumContrato(Ano: String): Integer;
function ThereIsFieldTable(const ATableName, AFieldName: String): Boolean;
...
Begin
   ...
   sCampoPesq := Trim(sCampoPesq);
   sTextPesq  := Trim(sTextPesq);
   oQry1 := TSQLQuery.Create(Nil);
   oQry1.SQLConnection := dmPrincipal.SConPrincipal;
   oQry1.SQL.Clear;
...
begin
  ...
  sRazaoSocialPrincipal := glb_sEmpresa;
  oQry1 := TSQLQuery.Create(Nil);
  oQry1.SQLConnection := dmPrincipal.SConPrincipal;
  ...
  try
    ...
    begin
       oQry1.SQL.Add('SELECT ID_UNID_GESTORA FROM SUB_UNID_ORCAMENT '+
          'WHERE ID = '+IntToStr(iSubUnidOrcam));
       oQry1.Open;
       iUnidGestora := oQry1.FieldByName('ID_UNID_GESTORA').Value;
       oQry1.Close;
       oQry1.SQL.Clear;
    end;
    oQry1.SQL.Add('SELECT RAZAO_SOCIAL, CNPJ, cnpj_principal, cnpj_da_gps '+
      'FROM UNID_GESTORA WHERE ID = '+IntToStr(iUnidGestora));
    oQry1.Open;
    sRazaoSocialUG  := '';
    if not oQry1.FieldByName('RAZAO_SOCIAL').IsNull then
       sRazaoSocialUG  := oQry1.FieldByName('RAZAO_SOCIAL').Value;
    if oQry1.FieldByName('CNPJ_da_gps').Value = '0' then // cnpj principal
       sCNPJ := oQry1.FieldByName('CNPJ').Value
    else
       sCNPJ := oQry1.FieldByName('CNPJ_principal').Value;

    slRet.Add(sCNPJ);
    if sRazaoSocialUG <> '' then
        ...
        begin
           if sPIS_PASEP = oQry1.FieldByName('PIS_PASEP_PF').Value then
              if not Confirma('JÁ EXISTE UM SERVIDOR COM ESTE CPF E PIS_PASEP:'+#13+
                          'ID. :'+FormatString('999999-9',oQry1.FieldByName('ID').AsString,False)+
...
begin
  ...
  SQL_Text := 'update '+Tabela+' set oper_usuario = '+QuotedStr('3')+', '+
    'id_sessao = '+IntToStr(glb_iSessao)+' where '+ClausuraWhere;

  dmPrincipal.SConPrincipal.ExecuteDirect(SQL_Text);

  // Executa uma Aleração apenas no Campo 'Oper_Usuario = 3' para que depois possa identificar
  // a Sessao e Posteriormente o Usuário que Excluiu
...
begin
  ...
  Qry1.SQLConnection := dmPrincipal.SConPrincipal;
  Qry1.SQL.Add('select coalesce(count(id_servidor),0)');
  Qry1.SQL.Add('from servidor_ctro_admin');
  Qry1.SQL.Add('where (id_servidor = '+Trim(IdServidor)+') and (id_lancto_origem = '+
                Trim(IdLanctoCtroOrig)+')');
  try
    Qry1.Open;
    Result := IntToStr(Qry1.Fields[0].AsInteger +1);
  finally
    Qry1.Close;
    FreeAndNil(Qry1);
  end;
...
begin
  ...
  oQry := TSQLQuery.Create(Nil);
  oQry.SQLConnection := dmPrincipal.SConPrincipal;
  sSQL_Text := 'select max(numero) as num_ctro from servidor_ctro_admin '+
     'where ano = '+QuotedStr(Ano);
  oQry.SQL.Add(sSQL_Text);
  try
    oQry.Open;
    if oQry.FieldByName('num_ctro').IsNull then
       Result := 1
    else
       try
         Result := oQry.FieldByName('num_ctro').AsInteger+1;
       except
         Result := 1;
       end;
  finally
...
begin
  ...
  qryBusca.SQLConnection := dmPrincipal.SConPrincipal;
  ...
  try
    ...
    begin
      ...
      SQL.Add('where f.rdb$relation_name = ' + QuotedStr(AnsiUpperCase(Trim(ATableName))) );
      SQL.Add('  and f.rdb$field_name    = ' + QuotedStr(AnsiUpperCase(Trim(AFieldName))) );
      SQL.EndUpdate;
      Open;
