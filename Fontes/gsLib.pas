...
Function MdnoConfirma(sTexto,Acao: String): Boolean;
Function ConfirmaDelete(sTexto, sTexto2: String): Boolean;
...
Function FormatString(sMask, sString: String; lZeroLeft: Boolean): String;
...
Function ConvertDateSaveDirect(sText: string): String;
Function ConvertIntegerSaveDirect(iVal: Integer): String;
function PermissoesUsuario(iIdUsuario: integer; sNomeTab: String): string;
function PositionChar(CharLocate, Texto: String; Ordem: SmallInt): SmallInt;
function ContaMesesPeriodo(AnoMesIni,AnoMesFim: String): Integer;
function DescrVincluloTCM(Id: Integer; Efetivo : String): String;

implementation

uses Variants, udmPrincipal, VarGlobais, UtilsDb, Windows,
  SelecionaOrgao, unMensagem;
...
Begin
  ...
  sFlags := IntToStr(iFlags);

  frmMensagem := TfrmMensagem.Create(Nil);
  frmMensagem.TituloMsg   := '';
  frmMensagem.SubTituloMsg:= 'ATEN«√O !!!';
  frmMensagem.TextoMsg    := sTexto;
  if sFlags = '64' then
     frmMensagem.TipoMsg  := 'INFORM'
  else
  if sFlags = '16' then
     frmMensagem.TipoMsg  := 'ERRO'
  else
  if sFlags = '48' then
     frmMensagem.TipoMsg  := 'ALERTA';
...
Begin
  ...
  frmMensagem := TfrmMensagem.Create(Nil);
  frmMensagem.TituloMsg   := 'CONFIRMA«√O';
  frmMensagem.SubTituloMsg:= 'ATEN«√O !!!';
  frmMensagem.TipoMsg     := 'CONFIRMA';
  frmMensagem.TextoMsg    := sTexto;
  frmMensagem.ShowModal;

  Result := frmMensagem.ConfirmaAcao;
...
Begin
  ...
  frmMensagem := TfrmMensagem.Create(Nil);
  frmMensagem.TituloMsg   := 'EXCLUS√O';
  frmMensagem.SubTituloMsg:= 'ATEN«√O !!!';
  frmMensagem.TipoMsg     := 'DELETE';
  frmMensagem.TextoMsg    := sTexto;
  frmMensagem.TextoMsg2   := sTexto2;
...
Var
   x: Extended;
begin
   //x := 51 + ((glb_iWidth - iLargura) / 2);
   //x := ((Screen.Width - iLargura) / 2) - 74;
   x := ((Screen.Width - iLargura) / 2);
   Result := Inteiro(x);
End;
...
begin
   x := 0;
   //x := 54 + ((gWinResHeight - iAltura) / 2);
   x := ((gWinResHeight - iAltura) / 2);
   Result := Inteiro(x - 38);
   if Result < 0 then
      Result := Result + 15;
End;
...
Begin
 If sTipo = 'F' Then
  Result := FormatString('999.999.999-99', sCPF_CNPJ, True)
 Else
  Result := FormatString('99.999.999/9999-99', sCPF_CNPJ, True);
End;
...
begin
  ...
  if iDiaFim < 30 then
     ...
     iQtdDiasFim := 0;
  if (iQtdDiasIni = 0) and (iQtdDiasFim = 0) then
     Inc(iQtdMeses);
...
Begin
  ...
  //Mensagem(Result,'Servidor e Banco',MB_OK+MB_ICONQUESTION);

  {
  begin
     Mensagem('Falta o Arquivo de InicializaÁ„o do Sistema ...',
...
begin
  ...
  {
  if Length(Trim(sLinReg)) = 6 then
     Result := LeftStr(sLinReg,6)+' 0N'
  else
  if Length(Trim(sLinReg)) = 7 then
     Result := LeftStr(sLinReg,6)+' '+RightStr(sLinReg,1)+'N'
  else
     Result := LeftStr(sLinReg,6)+' '+RightStr(sLinReg,2);
  }
end;
...
begin
  ...
  //sPathAtuaizaExe := Pesquisa('PASTA_PADRAO','ID','1','ATUALIZA_EXE','');
...
begin
  ...
  ShellExecute(HND_Principal,'Open',PChar(sExe+'.bat'),nil,nil,SW_HIDE);
...
Var
   sTemp: string;
begin
   sTemp := LeftStr(sData,2)+sNovoSeparador+Copy(sData,4,2)+sNovoSeparador+RightStr(sData,4);
   Result := sTemp;
end;
...
begin
   ...
   Result := 'Null';
   if (sText = '') or (sText = '00/00/0000') or (EmptyData(sText)) then Exit;

   Try
     Result := 'Cast('+QuotedStr(LeftStr(sText,2)+'.'+Copy(sText,4,2)+'.'+RightStr(sText,4))+' as date)';
   except
     Result := 'Null';
   End;
   if Trim(Result) = '' then
...
function PermissoesUsuario(iIdUsuario: integer; sNomeTab: String): string;
Var
  oQryUserPermTab: TSQLQuery;
  sTextSQL       : String;
  aPermRet       : Array[1..3] of string[1];
begin

  sTextSQL := 'SELECT INCLUIR,ALTERAR,EXCLUIR FROM USUARIO_PERMISSAO_TABELA '+
     'WHERE (ID_USUARIO = '+IntToStr(iIdUsuario)+') AND (NOME_TABELA = upper('+QuotedStr(sNomeTab)+'))';
  try
     oQryUserPermTab := TSQLQuery.Create(Nil);
     oQryUserPermTab.SQLConnection := dmPrincipal.SConPrincipal;
     oQryUserPermTab.SQL.Add(sTextSQL);
     oQryUserPermTab.Open;
     aPermRet[1] := oQryUserPermTab.FieldByName('INCLUIR').Value;
     aPermRet[2] := oQryUserPermTab.FieldByName('ALTERAR').Value;
     aPermRet[3] := oQryUserPermTab.FieldByName('EXCLUIR').Value;
  finally
     oQryUserPermTab.Close;
     FreeAndNil(oQryUserPermTab);
  end;

  Result := aPermRet[1]+aPermRet[2]+aPermRet[3];
...
begin
  ...
  Conta   := 1;
  Conta2  := 0;
  while (Conta <= TamTexto) do
  begin

    CharTmp := Copy(Texto,Conta,1);
    if CharTmp = CharLocate then
    begin
       Inc(Conta2);
       if Conta2 = Ordem then
       begin
          Result := Conta;
          Break;
       end;
...
begin
  ...
  AnoMesTmp:= AnoMesIni;
  Conta    := 0;
  while (AnoMesTmp <= AnoMesFim) do
  begin
    Inc(Conta);
    MesAnoTmp := InverteAnoMes(AnoMesTmp,'2');
    AnoMesTmp := InverteAnoMes(IncMesAno(MesAnoTmp),'1');
  end;
...
function DescrQualificSIOPE(Cod: SmallInt) : String;
begin

   case Cod of
      1: Result := 'CRECHE';
      2: Result := 'PR…-ESCOLA';
      3: Result := 'FUNDAMENTAL 1';
      4: Result := 'FUNDAMENTAL 2';
      5: Result := 'SEM DESCRI«√O';
      6: Result := 'PROFISSIONAL';
      7: Result := 'ADMINISTRATIVO';
   end;
...
begin
   ...
   if (Efetivo = 'S') or
      ...
      Result := 'MEMBRO DE CONSELHO MUNICIPAL'
   else If (Id = 50) or
           (Id = 51) or
           (Id = 52) then
      Result := 'INATIVO/PENSIONISTA'
   else If Id = 35 then
      Result := 'PROCURADOR GERAL'
   else
      Result := 'OUTRO';
